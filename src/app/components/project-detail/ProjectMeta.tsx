...
export default function ProjectMeta({ meta }: { meta: ProjectMetaType }) {
  ...
  return (
    <section ref={ref} className="relative py-10 bg-[var(--nx-black-alt)]">
          ...
          {metaLabels.map((item, i) => (
            <motion.div
              ...
              className={`py-6 md:py-8 px-4 md:px-8 ${
                i < metaLabels.length - 1
                  ? "md:border-r md:border-[var(--nx-border)]"
                  : ""
              }`}
            >
              <span
              ...
              <span
                className="text-[13px] tracking-[0.1em] uppercase text-[var(--nx-ivory)]"
                style={{ fontFamily: "var(--font-mono)" }}
              >
                {meta[item.key]}
              </span>
